...
def get_nouns_verbs(sentence):
    ...
    original_analysis = kkma.pos(sentence)
    ...
    def validate(self, attrs):
        ...
        if mouse_over not in sentence_tokens:
            raise serializers.ValidationError(
                "mouse_over must be a token in the sentence provided."
            )
...
class KoreanWordAnalyze(APIView):
    ...
    def post(self, request):
        ...
        filtered_analysis, original_analysis = get_nouns_verbs(new_sentence)
        ...
        if from_index_heuristic:
            return get_found_response(from_index_heuristic)
        ...
        if from_prefix_heuristic:
            return get_found_response(from_prefix_heuristic)
        ...
        if from_agglutinated_reconstruction:
            return get_found_response(from_agglutinated_reconstruction)
