...
class HanjaSearchTests(DbDataMixin, APITestCase):
  ...
  def test_filter_params(self):
    ...
    param_tests = {
      "decomposition": {
        ...
        "invalid": [
          "thisiswaytoolongandwouldnevermatchanything"
        ]
      },
      "radical": {
        "valid": [
      ...
      "strokes": {
        ...
        "invalid": [
          "gtenotnum", "not7.", "toolongofaparam"
        ]
      },
      "grade_level": {
        "valid": [
      ...
      "exam_level": {
        ...
        "invalid": [
          "norelop", "thisiswaytoolong", "gte!notalevel!"
        ]
      },
      "nonexistent_filter": {
        "valid": [],
  ...
  def test_search_hanja_reading_before_meaning(self):
    ...
    for i in range (response_json['count']):
      self.assertEqual(response_json['results'][i]['character'], expected_results_order[i])
