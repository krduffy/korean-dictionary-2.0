...
def error_if_too_long(param_name, param, max_length):
  if len(param) > max_length:
    return get_bad_request(f"Query parameter '{param_name}' cannot exceed length {max_length}")
  return None
...
def get_hanja_search_param_error(query_params):
  ...
  allowed_params = ["page", "search_term", "decomposition", "radical", "strokes", "grade_level", "exam_level"]
  for key in query_params:
    if key not in allowed_params:
      return get_bad_request(f"Unknown query parameter '{key}'")
