...
from korean.models import KoreanHeadword, Sense
import os
import django
import json
import re
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument(
            "--drop",
            action="store_true",
            help="Drops all KoreanHeadwords and Senses before execution.",
        )
        parser.add_argument(
            "fname",
            type=str,
            help="Indicates which file should be added. Use 'all' for all files.)",
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if drop_all:
            KoreanHeadword.objects.all().delete()
            Sense.objects.all().delete()
            self.stdout.write("Dropped all KoreanHeadwords and Senses.")

        dict_dir = "korean/management/dictionary_data/우리말샘"
        json_files = []
        BATCH_SIZE = 5000
        batches_added = 0
        ...
        if file == "all":
            json_files = [
                os.path.join(dict_dir, fileindir) for fileindir in os.listdir(dict_dir)
            ]
        else:
            ...
            with open(dict_file, mode="r", encoding="utf-8") as raw_file:
                ...
                for sense_structure in dict_json["channel"]["item"]:
                    ...
                    if len(senses_to_add) >= BATCH_SIZE:
                        ...
                        batches_added += 1
                        self.stdout.write(
                            f"Created {len(senses_to_add)} senses in bulk. (cumulative {BATCH_SIZE * batches_added})"
                        )
        ...
        if senses_to_add:
            Sense.objects.bulk_create(senses_to_add)
            self.stdout.write(
                f"Created {len(senses_to_add)} senses in bulk. "
...
def get_as_sense(channel_item):
    ...
    additional_info_or_none = {
        info: senseinfo.get(info, None) for info in additional_info_choices
    }

    sense_additional_info = {
        info_key: info_value
        for info_key, info_value in additional_info_or_none.items()
    ...
    if "<DR />" in sense_def:
        try:
            sense_def = sense_def.replace(
                ...
                + ", ".join(
                    region_dict["region"]
                    for region_dict in sense_additional_info["region_info"]
                )
                + ").",
            )
        except:
...
def add_word(wordinfo: dict, word_target_code: int, history_info: dict | None) -> int:
    ...
    if KoreanHeadword.objects.filter(target_code=word_target_code).exists():
        return 1
    ...
    new_word = KoreanHeadword(
        target_code=word_target_code,
        word=word,
        origin=origin,
        word_type=word_type,
