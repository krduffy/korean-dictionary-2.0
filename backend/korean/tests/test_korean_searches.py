...
class KoreanHeadwordSearchTests(DbDataMixin, APITestCase):
    ...
    def test_search_type_param_staff(self):
        ...
        for search_type in allowed_search_types:
            with self.subTest(case=search_type):
                response = self.client.get(
                    self.search_url, {"search_term": "가", "search_type": search_type}
                )
        ...
        for term in successful_terms:
            with self.subTest(case=term):
                success_response = self.client.get(
                    self.search_url,
                    {"search_term": term, "search_type": self.filler_search_type},
                )
    ...
    def test_get_word_exact(self):
        ...
        for case in cases:
            with self.subTest(case=case):
                success_response = self.client.get(
                    self.search_url,
                    {"search_term": case["word"], "search_type": "word_exact"},
                )
