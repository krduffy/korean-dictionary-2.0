...
export interface CacheContextProviderArgs {
  children: ReactNode;
  cacheCapacity: number;
}

export const CacheContextProvider: React.FC<CacheContextProviderArgs> = ({
  children,
  cacheCapacity,
}) => {
  const cache = {};
   ...
   */
  const processRequest = (url, method, additionalInfo) => {
    /*
   ...
   */
  const cachePut = (url, response, ok) => {
    if (Object.keys(cache).includes(url)) {
      cache[url].lastAccessed = ++counter;
    } else {
      cache[url] = {
  ...
  const getSearchesMatchingWord = (urlRegex, word) => {
    ...
    Object.keys(cache).forEach((url) => {
      if (
        ...
        urlMatches(url, "api\\/search_korean\\/\\?page=\\d+&search_term=", word)
      ) {
        searches.push(url);
      }
    });
